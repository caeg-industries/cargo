...
/// This is used in contexts where the full subcrate name needs to be
/// a valid filename, like writing a binary to the filesystem. Some
/// files of these files are generated by Rustc, so right now this
/// actually needs to be the same as the rust code replacement. It may
/// be a good idea to just merge the two.
pub const SUBCRATE_DELIMETER_FILENAME_REPLACEMENT: &str = "_";
/// Keep this in sync with the filename version of the delimeter used
/// by crates.io. It's used for the tarball uploaded when publishing crates.
pub const SUBCRATE_DELIMETER_REGISTRY_FILENAME_REPLACEMENT: &str = "~";
...
impl Target {
    ...
    pub fn file_safe_crate_name(&self) -> String {
        self.name()
            .replace("-", "_")
            .replace(SUBCRATE_DELIMETER, SUBCRATE_DELIMETER_FILENAME_REPLACEMENT)
    }

    /// I think this might be the same as crate_name?
    pub fn rust_code_safe_name(&self) -> String {
        self.name()
            .replace("-", "_")
